...
public class DonacionesServiceRpc extends DonacionesServiceGrpc.DonacionesServiceImplBase {
        ...
        public void enviarSolicitud(SolicitudDonacionRequest request, StreamObserver<Respuesta> responseObserver) {
                System.out.println("📥 Solicitud recibida en gRPC: " + request.getIdOrganizacion() + ", "
        ...
        public void listarSolicitudes(Vacio request, StreamObserver<ListaSolicitudesResponse> responseObserver) {
                ...
                List<SolicitudDonacionDTO> solicitudesManuales = donacionesService.obtenerSolicitudesExternas();
                List<SolicitudDonacionAutomaticoDTO> solicitudesAutomaticas = donacionesService
                ...
                solicitudesManuales.forEach(solicitud -> {
                        ...
                        solicitud.getDonaciones().forEach(item -> solicitudProto.addDonaciones(
                ...
                solicitudesAutomaticas.forEach(auto -> {
                        SolicitudDonacionRequest.Builder solicitudProto = SolicitudDonacionRequest.newBuilder()
                                        .setIdOrganizacion(auto.getIdOrganizacion())
                                        .setIdSolicitud(auto.getIdSolicitud());

                        auto.getDonaciones().forEach(item -> solicitudProto.addDonaciones(
                                        ItemSolicitud.newBuilder()
                                                        .setCategoria(item.getCategoria())
                                                        .setDescripcion(item.getDescripcion())
                                                        .build()));
        ...
        public void enviarOferta(OfertaDonacionRequest request, StreamObserver<Respuesta> responseObserver) {
                donacionesService.ofrecerDonaciones(new OfertaDonacionDTO(
                                ...
                                request.getDonacionesList().stream()
                                                .map(item -> new com.myorg.kafka_module.dto.ItemDonacionDTO(
                                                                item.getCategoria(), item.getDescripcion(),
                                                                item.getCantidad()))
                                                .collect(Collectors.toList())));
        ...
        public void listarOfertas(Vacio request, StreamObserver<ListaOfertasResponse> responseObserver) {
                ...
                donacionesService.obtenerOfertasRecibidas().forEach(oferta -> {
                        ...
                        oferta.getDonaciones().forEach(item -> ofertaProto.addDonaciones(
        ...
        /*
         * @Override
         * public void enviarTransferencia(TransferenciaDonacionRequest request,
         ...
         * request.getDonacionesList().stream()
         * .map(item -> new
         * com.myorg.kafka_module.dto.ItemDonacionDTO(item.getCategoria(),
